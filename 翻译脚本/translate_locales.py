        ...
        if isinstance(value, dict):
            ...
            for i, item in enumerate(value):
                if isinstance(item, str):
                    acc[f"{path_key}.{i}"] = item
                elif isinstance(item, (dict, list)):
                    flatten_object({str(i): item}, path_key, acc)
                else:
                    # Fallback for non-string/non-obj items in list
                    acc[f"{path_key}.{i}"] = str(item)
        elif isinstance(value, str):
            acc[path_key] = value
        else:
            # Fallback for other leaf types (bool, int, etc.)
            acc[path_key] = str(value)
    return acc
...
def unflatten_object(flat: Dict[str, str]) -> dict:
    ...
    def dict_to_list_where_possible(obj):
        if not isinstance(obj, dict):
            return obj
        
        # Recurse first
        for k, v in obj.items():
            obj[k] = dict_to_list_where_possible(v)
        ...
        if all(k.isdigit() for k in keys):
            ...
            # Check if keys are continuous from 0
            if [int(k) for k in sorted_keys] == list(range(len(keys))):
                return [obj[k] for k in sorted_keys]
        
        return obj

    return dict_to_list_where_possible(result)
...
def sanitize_json_text(raw: str) -> str:
    ...
    # Try to find array if object not found or if array seems more prominent
    ...
    if first_bracket >= 0 and (first_brace < 0 or first_bracket < first_brace):
        ...
        raise ValueError("Model response does not contain a valid JSON object or array.")

    if start_idx < 0 or end_idx < 0 or end_idx <= start_idx:
        raise ValueError("Model response contains invalid JSON bounds.")
    ...
    # 2. Fix common LLM JSON syntax errors
    ...
    # Common case: a list of strings but some quotes or commas are weird
    # If it's a list, we can try to be very aggressive
    if is_array:
...
def request_chat_completion(
    *,
    ...
    try:
        ...
        raise ValueError(f"Translation API request failed with HTTP {err.code}: {error_body}") from err
    except error.URLError as err:
        # Handle DNS / Connection errors specifically
        raise ConnectionError(f"Network error (check your internet/DNS): {err.reason}") from err
    except Exception as err:
        raise RuntimeError(f"Unexpected request error: {err}") from err
...
def verify_model_endpoint(*, base_url: str, api_key: str, model: str, timeout_sec: int) -> None:
    ...
    _ = request_chat_completion(
        ...
        model=model,
        messages=[
            {"role": "user", "content": "Return exactly: OK. OK"},
        ],
        temperature=0.0,
...
def is_locale_fully_adapted(*, output_path: Path, lang: str, source_strings: Dict[str, str]) -> bool:
    ...
    allow_cjk = lang in {"zh_CN", "zh_TW", "zh-CN", "zh-TW", "ja", "ko"}
    ...
    *,
    ...
    system_prompt = " ".join(
        [
            ...
            f'Return ONLY one valid JSON object in format {{"values":[...]}}.',
            f"The 'values' array MUST have EXACTLY {len(source_values)} items in the same order as the input.",
            "Each item should be the translated string.",
            "Preserve placeholders like {{count}} exactly.",
    ...
    *,
    ...
    system_prompt = " ".join(
        [
            "You are a professional software localization translator.",
            f"Translate UI messages into {language_name}.",
            f"Return ONLY one valid JSON object with EXACTLY {len(source_keys)} keys.",
...
def translate_one_language(
    *,
    ...
    def process_chunk(current_chunk: List[Tuple[str, str]], level: int = 0) -> Dict[str, str]:
        chunk_obj = dict(current_chunk)
        try:
            return translate_chunk(
                lang=lang,
                chunk_object=chunk_obj,
                base_url=base_url,
                api_key=api_key,
                model=model,
                temperature=temperature,
            ...
            # If it's a pure network error (DNS, connection), splitting won't help.
            # Only split for parsing/logic/size errors.
            if isinstance(e, ConnectionError):
                raise

            if level < 8 and len(current_chunk) > 1:
                mid = len(current_chunk) // 2
                print(f"[{lang}] chunk failed ({e}), splitting into sub-chunks (level {level+1})...", flush=True)
                left = process_chunk(current_chunk[:mid], level + 1)
                right = process_chunk(current_chunk[mid:], level + 1)
                left.update(right)
                return left
            raise
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        for key, _ in chunk:
            if key in translated_chunk:
                translated_flat[key] = translated_chunk[key]
